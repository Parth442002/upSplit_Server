...
export async function formatDebtMap(group: GroupDocument) {
  ...
  for (const [debtKey, amount] of group.DebtMap.entries()) {
    ...
    if (sender && receiver) {
      const senderName = sender.username; // Replace with the appropriate field in your User model
      const receiverName = receiver.username; // Replace with the appropriate field in your User model

      // Determine who owes whom and the amount
